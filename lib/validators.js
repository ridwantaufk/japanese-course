...
export async function validateForeignKey(query, tableName, columnName, value) {
  ...
  try {
    ...
    return {
      ...
      message: result.rowCount > 0 ? '' : `Referenced ${tableName} ID ${value} does not exist`
    };
  } catch (e) {
    return { valid: false, message: `Error checking foreign key: ${e.message}` };
  }
}
...
export function validateField(field, value) {
  ...
  switch (field.type) {
    ...
    case 'number':
      const numCheck = validateNumberRange(
        value, 
        field.min, 
        field.max
      );
...
export function validateImportRow(fields, row, rowNumber) {
  ...
  fields.forEach(field => {
    ...
    if (!result.valid) {
      ...
      warnings.push(`Row ${rowNumber}, ${field.label}: Value converted from "${value}" to "${result.value}"`);
    }
