...
async function getExamData(id) {
  try {
    ...
    const sectionsRes = await query(
      "SELECT * FROM jlpt_exam_sections WHERE exam_id = $1 ORDER BY section_order ASC",
      [id]
    );
  ...
  if (data.sections.length === 0 || totalQuestions === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center text-center p-8 bg-slate-50 dark:bg-[#020617]">
        <div>
          <h1 className="text-3xl font-black text-slate-300 mb-2">
            Construction In Progress
          </h1>
          <p className="text-slate-500 max-w-md mx-auto">
            The exam <strong>{data.exam.title_id}</strong> structure exists, but
            the questions haven&apos;t been uploaded to the database yet.
          </p>
        </div>
      </div>
    );
