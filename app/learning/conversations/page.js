  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center space-y-4">
        <h1 className="text-4xl font-black text-slate-900 dark:text-white">Conversations</h1>
        <p className="text-slate-500 dark:text-slate-400">Practice real-life dialogues.</p>
      </div>
        ...
        {conversations.map((conv) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className={`px-3 py-1 rounded-full text-xs font-bold bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300`}>
                {conv.jlpt_level || 'N/A'}
              </span>
              {conv.topic && <span className="text-xs font-bold uppercase tracking-wider text-slate-400">{conv.topic}</span>}
            </div>
            ...
            <div className="flex items-center gap-4 text-xs font-medium text-slate-400 border-t border-slate-100 pt-4 dark:border-white/5">
               {conv.duration_seconds && (
                 <span className="flex items-center gap-1"><Clock size={14} /> {Math.floor(conv.duration_seconds / 60)} min</span>
               )}
               <span className="flex items-center gap-1"><BarChart size={14} /> {conv.difficulty_score || 1}/5</span>
            </div>
          </Link>
        ))}
      </div>
      ...
      {conversations.length === 0 && (
        <div className="text-center py-20 text-slate-400">No conversations found.</div>
      )}
    </div>
  );
}