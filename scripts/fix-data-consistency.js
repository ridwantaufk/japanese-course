...
async function fixDataConsistency() {
  try {
    ...
    if (updates.length > 0) {
      ...
      for (const update of updates) {
        ...
        if (updates.indexOf(update) % 50 === 0) {
          console.log(
            `   ✓ Fixed ${updates.indexOf(update) + 1}/${updates.length}...`
          );
        }
      }
    ...
    for (const row of samples.rows) {
      ...
      console.log(
        `   Onyomi: ${JSON.stringify(row.onyomi, null, 2).substring(0, 100)}...`
      );
      console.log(
        `   Kunyomi: ${JSON.stringify(row.kunyomi, null, 2).substring(
          0,
          100
        )}...`
      );
