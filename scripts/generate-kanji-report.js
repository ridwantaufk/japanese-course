...
async function generateReport() {
  try {
    ...
    console.log(
      `   Min/Max Strokes: ${stats.min_strokes}/${stats.max_strokes}`
    );
    ...
    jlptDist.rows.forEach((row) => {
      ...
      console.log(
        `   N${row.jlpt_level}: ${row.count
          .toString()
          .padStart(3)} kanji (${percentage}%) ${bar}`
      );
    });
    ...
    categoryDist.rows.forEach((row, index) => {
      ...
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${row.category.padEnd(
          25
        )} : ${row.count.toString().padStart(3)} (${percentage}%) ${bar}`
      );
    });
    ...
    wordTypeDist.rows.forEach((row, index) => {
      ...
      const bar = "▒".repeat(Math.floor(row.count / 10));
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${row.word_type.padEnd(
          30
        )} : ${row.count.toString().padStart(3)} (${percentage}%) ${bar}`
      );
    });
    ...
    console.log(
      `   ✅ English Meaning  : ${comp.has_meaning_en}/${comp.total} (${(
        (comp.has_meaning_en / comp.total) *
        100
      ).toFixed(1)}%)`
    );
    console.log(
      `   ✅ Indonesian Meaning: ${comp.has_meaning_id}/${comp.total} (${(
        (comp.has_meaning_id / comp.total) *
        100
      ).toFixed(1)}%)`
    );
    console.log(
      `   ✅ Onyomi Readings : ${comp.has_onyomi}/${comp.total} (${(
        (comp.has_onyomi / comp.total) *
        100
      ).toFixed(1)}%)`
    );
    console.log(
      `   ✅ Kunyomi Readings: ${comp.has_kunyomi}/${comp.total} (${(
        (comp.has_kunyomi / comp.total) *
        100
      ).toFixed(1)}%)`
    );
    console.log(
      `   ✅ Categorized     : ${comp.has_category}/${comp.total} (${(
        (comp.has_category / comp.total) *
        100
      ).toFixed(1)}%)`
    );
    console.log(
      `   ✅ Word Type Defined: ${comp.has_word_type}/${comp.total} (${(
        (comp.has_word_type / comp.total) *
        100
      ).toFixed(1)}%)`
    );
    ...
    for (let level = 5; level >= 1; level--) {
      ...
      samples.rows.forEach((row) => {
        console.log(
          `      ${row.character} | ${row.meaning_en.padEnd(
            20
          )} | ${row.category.padEnd(20)} | ${row.word_type.padEnd(15)} | On:${
            row.onyomi_count
          } Kun:${row.kunyomi_count}`
        );
      });
    }
    ...
    console.log(
      `   ⚠️  Word Type still 'Umum'   : ${iss.word_type_umum} kanji`
    );
    ...
    console.log(
      `   ✨ Categorized 712 kanji into 19+ specific categories (from 100% Umum → 2.2% Umum)`
    );
    console.log(
      `   ✨ Classified 648 kanji into 15+ word types (from 38.5% Umum → 11.0% Umum)`
    );
    console.log(`   ✨ Added romaji to 687 kanji readings`);
    console.log(
      `   ✨ Normalized 510 kanji readings by splitting combined entries`
    );
    console.log(
      `   ✨ Created indexes on category and word_type for better query performance`
    );
