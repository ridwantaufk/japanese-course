...
async function analyzeExamples() {
  try {
    ...
    console.log(`📊 Kanji with LESS THAN 2 examples: ${needMoreExamples.rows.length}`);
    ...
    missingExamples.rows.slice(0, 20).forEach(row => {
      console.log(`${row.character} (ID: ${row.id}) - ${row.meaning_en} [N${row.jlpt_level}]`);
    });
    ...
    console.log(`✅ Exported ${needMoreExamples.rows.length} kanji needing more examples to kanji-need-more-examples.json`);
