...
async function analyzeKanjiData() {
  try {
    ...
    const checks = [
      {
        name: "Missing onyomi",
        query:
          "SELECT COUNT(*) FROM kanji WHERE onyomi IS NULL OR TRIM(onyomi) = ''",
      },
      {
        name: "Missing kunyomi",
        query:
          "SELECT COUNT(*) FROM kanji WHERE kunyomi IS NULL OR TRIM(kunyomi) = ''",
      },
      {
        name: "Missing meaning_en",
        query:
          "SELECT COUNT(*) FROM kanji WHERE meaning_en IS NULL OR TRIM(meaning_en) = ''",
      },
      {
        name: "Missing meaning_id",
        query:
          "SELECT COUNT(*) FROM kanji WHERE meaning_id IS NULL OR TRIM(meaning_id) = ''",
      },
      {
        name: "Category = Umum",
        query: "SELECT COUNT(*) FROM kanji WHERE category = 'Umum'",
      },
      {
        name: "Word_type = Umum",
        query: "SELECT COUNT(*) FROM kanji WHERE word_type = 'Umum'",
      },
    ...
    jlptDist.rows.forEach((row) => {
      console.log(
        `N${row.jlpt_level}: ${row.total} kanji (${row.umum_category} uncategorized, ${row.umum_type} untyped)`
      );
    });
    ...
    console.log(
      `✅ Exported ${fullData.rows.length} records to kanji-needs-improvement.json`
    );
  } catch (error) {
