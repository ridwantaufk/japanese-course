...
export default function ExamRunner({ exam, sections }) {
  ...
  useEffect(() => {
    if (!isFinished && Object.keys(answers).length > 0) {
      const autoSave = setInterval(() => {
        saveExamInProgress(exam.id, currentSectionIndex, answers, Date.now());
      }, 30000); // 30 seconds

      return () => clearInterval(autoSave);
    }
  }, [answers, currentSectionIndex, exam.id, isFinished]);

  // Restore progress on mount
  useEffect(() => {
    const progress = getExamProgress();
    if (progress.inProgress[exam.id]) {
      const saved = progress.inProgress[exam.id];
      setAnswers(saved.answers || {});
      setCurrentSectionIndex(saved.currentSection || 0);
    }
  }, [exam.id]);
  ...
  if (isFinished) {
    ...
    sections.forEach((s) => {
      let sectionScore = 0;
      let sectionTotal = s.questions.length;
    ...
    // Save exam completion
    saveExamCompletion(exam.id, rawScore, totalQs, sectionResults);
      ...
      <div className="max-w-2xl mx-auto text-center py-20 space-y-8 animate-in zoom-in duration-500">
        ...
        <div className="flex justify-center gap-4">
          <button
            ...
            className="px-6 py-3 rounded-xl border border-slate-200 hover:bg-slate-50 font-bold dark:border-white/10 dark:hover:bg-white/5"
          >
            Retake
          </button>
          <button
            onClick={() => (window.location.href = "/learning/jlpt")}
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-slate-900 text-white px-6 py-4 flex justify-between items-center shadow-md">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Section {currentSectionIndex + 1}: {currentSection.section_name_id}
          </p>
        </div>
        <div
          className={`flex items-center gap-2 font-mono text-xl font-bold ${
            timeLeft < 300 ? "text-red-400 animate-pulse" : "text-emerald-400"
          }`}
        >
          <Timer size={20} />
          {formatTime(timeLeft)}
      ...
      <div className="flex-1 pt-24 pb-32 max-w-4xl mx-auto w-full px-4 space-y-8">
        ...
        {currentSection.instructions_id && (
          <div className="bg-white dark:bg-[#0f172a] border-l-4 border-indigo-500 p-6 rounded-r-xl shadow-sm text-slate-700 dark:text-slate-300 text-sm leading-relaxed">
            <strong className="block text-indigo-600 dark:text-indigo-400 mb-2 uppercase text-xs tracking-wider">
              Instructions
            </strong>
            {currentSection.instructions_id}
          </div>
        ...
        <div className="space-y-6">
          {questions.map((q, idx) => {
            ...
            return (
              <div
                key={q.id}
                className="bg-white dark:bg-[#0f172a] p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-white/5"
              >
                <div className="flex gap-4 mb-6">
                  <span className="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 dark:bg-white/10 text-sm font-bold text-slate-600 dark:text-white">
                    {q.question_number}
                  </span>
                      ...
                      <div className="mb-4 p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/10">
                        <div className="flex justify-between items-start gap-3">
                          ...
                          {q.audio_url && (
                            <button
                              onClick={() =>
                                playAudio(q.audio_url, q.id, q.context_ja)
                              }
