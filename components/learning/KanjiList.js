...
export default function KanjiList({ initialData }) {
  ...
  const renderReading = (reading) => {
    ...
    if (typeof reading === 'object') return reading.reading || reading.kana || JSON.stringify(reading);
    return reading;
  };
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-400">Select your JLPT level to start learning.</p>
      ...
      <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
        {filteredData.map((k) => (
          ...
          >
            <span className="text-5xl font-black text-slate-800 dark:text-white mb-2">{k.character}</span>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider truncate px-2 max-w-full">{k.meaning_id || k.meaning_en}</span>
            <div className="absolute top-3 right-3 text-[10px] font-bold text-slate-300 dark:text-slate-600">{k.stroke_count}s</div>
          </button>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-3xl bg-white dark:bg-[#0f172a] shadow-2xl overflow-hidden">
            ...
            <div className="relative bg-gradient-to-br from-indigo-600 to-violet-700 p-8 text-center text-white shrink-0">
              ...
              <div className="text-8xl font-black mb-2">{selectedKanji.character}</div>
              ...
              <div className="mt-4 flex justify-center gap-4 text-sm font-medium text-white/80">
                <span className="bg-white/10 px-3 py-1 rounded-full">{selectedKanji.jlpt_level}</span>
                <span className="bg-white/10 px-3 py-1 rounded-full">{selectedKanji.stroke_count} Strokes</span>
              </div>

              <button 
                  onClick={() => playAudio(selectedKanji.character, selectedKanji.audio_url)}
            ...
            <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-8 border-b border-slate-100 dark:border-white/5 pb-8">
                <div>
                  <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-3">Onyomi (Chinese)</h3>
                  <div className="flex flex-wrap gap-2">
                    {selectedKanji.onyomi && selectedKanji.onyomi.length > 0 ? (
                        selectedKanji.onyomi.map((on, i) => (
                        <span key={i} className="px-3 py-1 rounded-lg bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-200 font-bold">
                            {renderReading(on)}
                        </span>
                        ))
                ...
                <div>
                  <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-3">Kunyomi (Japanese)</h3>
                  <div className="flex flex-wrap gap-2">
              ...
              <div>
                ...
                {loadingExamples ? (
                    ...
                    <div className="grid gap-3">
                        {examples.map((ex, i) => (
                            <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:border-indigo-200 transition-colors cursor-pointer" onClick={() => playAudio(ex.japanese_text, ex.audio_url)}>
                                <div>
                                    <FuriganaText text={ex.japanese_text} furigana={ex.furigana} className="text-lg font-bold text-slate-800 dark:text-white" />
                                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{ex.meaning_id}</p>
                                </div>
