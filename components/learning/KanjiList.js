...
export default function KanjiList({ initialData }) {
  ...
  const availableRadicals = useMemo(() => {
    return [
      "all",
      ...new Set(currentLevelData.map((k) => k.radical).filter(Boolean)),
    ].sort();
  }, [currentLevelData]);
  ...
  const availableCategories = useMemo(() => {
    return [
      "all",
      ...new Set(currentLevelData.map((k) => k.category).filter(Boolean)),
    ].sort();
  }, [currentLevelData]);
  ...
  const availableTypes = useMemo(() => {
    return [
      "all",
      ...new Set(currentLevelData.map((k) => k.word_type).filter(Boolean)),
    ].sort();
  }, [currentLevelData]);
  ...
  const filteredData = useMemo(() => {
    ...
    if (strokeFilter !== "all") {
      ...
      if (strokeFilter === "16+")
        data = data.filter((k) => k.stroke_count >= 16);
      else
        data = data.filter(
          (k) => k.stroke_count >= min && k.stroke_count <= max
        );
    }
    if (radicalFilter !== "all")
      data = data.filter((k) => k.radical === radicalFilter);
    if (categoryFilter !== "all")
      data = data.filter((k) => k.category === categoryFilter);
    if (typeFilter !== "all")
      data = data.filter((k) => k.word_type === typeFilter);
    return data;
  }, [
    currentLevelData,
    strokeFilter,
        ...
        if (active) {
          if (result.data && result.data.length > 0) {
            ...
            const fallbackRes = await fetch(
              `/api/admin/vocabulary?search=${encodeURIComponent(
                selectedKanji.character
              )}&limit=5`
            );
            const fallbackData = await fallbackRes.json();
            setExamples(
              (fallbackData.data || []).map((v) => ({
  ...
  return (
    <div className="space-y-8 pb-20">
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-sm font-bold">
            <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
              <span className="px-2 py-0.5 rounded bg-indigo-100 dark:bg-indigo-900/30 text-xs">
            ...
            {filteredData.length !== currentLevelData.length && (
              <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 animate-in fade-in slide-in-from-left-2">
                <span className="px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/30 text-xs">
                  {filteredData.length}
                </span>
                Showing
              </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {strokeRanges.map((range) => (
                    <button
                      ...
                      className={cn(
                        "px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all border cursor-pointer",
                        strokeFilter === range
                          ? "bg-purple-600 border-purple-600 text-white shadow-md"
                          : "bg-slate-50 dark:bg-white/5 border-transparent text-slate-500 hover:border-purple-300"
                      )}
                    >
                      {range}
                    </button>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-1 max-h-[80px] overflow-y-auto custom-scrollbar pr-2">
                  {availableRadicals.map((rad) => (
                    <button
                      key={rad}
                      onClick={() => setRadicalFilter(rad)}
                      className={cn(
                        "w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold transition-all border cursor-pointer",
                        radicalFilter === rad
                          ? "bg-pink-600 border-pink-600 text-white shadow-md"
                          : "bg-slate-50 dark:bg-white/5 border-transparent text-slate-500 hover:border-pink-300"
                      )}
                    >
                      {rad === "all" ? "All" : rad}
                    </button>
          ...
          {filteredData.length > 0 && (
            <div className="mt-2 animate-in zoom-in duration-300">
              <InteractiveKanjiQuiz kanjiData={filteredData} level={level} />
            </div>
          )}
        </div>
      </div>
      ...
      <div
        className={cn(
          "grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 transition-all duration-500",
      ...
      >
        {filteredData.map((k) => (
          <button
            ...
            className="group relative aspect-square flex flex-col items-center justify-center rounded-3xl bg-white shadow-sm ring-1 ring-slate-200 transition-all hover:-translate-y-1 hover:shadow-xl hover:ring-indigo-400 dark:bg-[#0f172a] dark:ring-white/10 dark:hover:ring-indigo-500 cursor-pointer"
          >
            <span className="text-5xl font-black text-slate-800 dark:text-white mb-2 group-hover:scale-110 transition-transform duration-300">
              {k.character}
            </span>
            ...
            <div className="absolute bottom-3 right-3 text-[8px] font-black text-indigo-500/40 uppercase tracking-tighter">
              {k.word_type?.slice(0, 5)}
            </div>
          </button>
        ))}
      </div>
      ...
      {filteredData.length === 0 && !isPending && (
        <div className="text-center py-20 bg-slate-50 dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-white/10">
          <p className="text-slate-400 font-bold text-lg">
            No Kanji matches your current filters.
          </p>
          <button
            onClick={() => {
              setStrokeFilter("all");
            ...
            className="px-6 py-2 rounded-full bg-slate-900 text-white text-sm font-bold mt-4 hover:bg-slate-800 dark:bg-white dark:text-slate-900 transition-all cursor-pointer"
          >
            Reset All Filters
          </button>
        </div>
            ...
            >
              <div className="relative bg-gradient-to-br from-indigo-600 to-violet-700 p-6 sm:p-8 text-center text-white shrink-0">
                ...
                <button
                  ...
                  className="mt-6 inline-flex items-center gap-2 rounded-full bg-white text-indigo-700 px-8 py-3 text-sm font-black shadow-xl hover:scale-105 transition-all active:scale-95 cursor-pointer"
                >
              ...
              <div className="flex-1 p-6 sm:p-8 space-y-8 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedKanji.onyomi?.length > 0 ? (
                        selectedKanji.onyomi.map((on, i) => (
                          <div
                            key={i}
                            className="flex flex-col bg-slate-50 dark:bg-white/5 p-2 rounded-xl border border-slate-100 dark:border-white/5 min-w-[60px] items-center"
                          >
                ...
                <div>
                  <h3 className="flex items-center gap-2 text-lg font-black text-slate-800 dark:text-white mb-4">
                    <Book size={20} className="text-indigo-500" /> Vocabulary
                    Examples
                  </h3>
                  {loadingExamples ? (
                    ...
                    <div className="grid gap-3">
                      {examples.map((ex, i) => (
                        <div
                          key={i}
                          className="group flex items-center justify-between p-4 rounded-2xl bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:border-indigo-300 transition-all cursor-pointer shadow-sm"
                          onClick={() =>
                        ...
                        >
                          <div>
                            <FuriganaText
                              ...
                              className="text-xl font-bold text-slate-800 dark:text-white group-hover:text-indigo-600 transition-colors"
                            />
                    ...
                    <div className="text-center py-10 bg-slate-50 dark:bg-white/5 rounded-2xl border-2 border-dashed border-slate-200 dark:border-white/10">
                      <p className="text-slate-400 text-sm font-bold">
                        No examples found for this Kanji.
                      </p>
                    </div>
                  )}
                </div>
