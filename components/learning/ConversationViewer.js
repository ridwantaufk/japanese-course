              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    className={`absolute top-2 right-2 p-1.5 rounded-full transition-all hover:scale-110 ${
                      playingAudio === line.id
                        ...
                        : "bg-indigo-500 text-white hover:bg-pink-500"
                    }`}
                  >
                    <Volume2 size={16} />
                  </button>
                  ...
                  {line.romaji && (
                    <div
                      className={`text-xs font-mono mt-1 ${
                        isLeft ? "text-slate-400" : "text-indigo-200"
                      }`}
                    >
                      {line.romaji}
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {line.word_breakdown.map((wb, idx) => (
                        <button
                          key={idx}
                          onClick={() =>
                            setActiveWord(activeWord === wb ? null : wb)
                          }
                  ...
                  {line.meaning_id && (
                    <p
                      className={`text-sm ${
                        isLeft ? "text-slate-500" : "text-indigo-100"
                      } border-t ${
                        isLeft ? "border-slate-100" : "border-indigo-500"
                      } pt-2 mt-3`}
                    >
        ...
        {activeWord && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-11/12 max-w-md bg-slate-900/95 text-white backdrop-blur-xl p-5 rounded-2xl shadow-2xl animate-in slide-in-from-bottom-4 zoom-in-95 border border-white/10">
            ...
            <div className="text-center">
              ...
              {activeWord.reading && (
                <p className="text-base font-medium text-pink-300 mb-1">
                  {activeWord.reading}
                </p>
              )}
              {activeWord.romaji && (
